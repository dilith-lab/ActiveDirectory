# Name: Add Group Members from SamAccountName
# Description: Add group members to a given group.
# Author: Dilith Achalan
# Date: 08/03/2023
# Version: 1.0.0
# Usage: PowerShell.exe -ExecutionPolicy Bypass -File "C:\Scripts\AddMembers.ps1" -GroupName [Group Name] -MemberList [csv file]
...
try{
    ...
    if (Test-Path $FileName -ErrorAction Stop) {
        ...
        foreach ($Member in $MemberList){
            Add-ADGroupMember -Identity $Group -Members $Member.SamAccountName -ErrorAction Stop
            Write-Host $Member.SamAccountName "added to the $GroupName group" -ForegroundColor Green
        }
        Write-Host "Group members added to $GroupName group" -ForegroundColor Yellow
